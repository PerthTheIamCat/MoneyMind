            ...
            (err, results) => {
                ...
                if (results.length > 0) {
                    ...
                    db.query(
                        ...
                        (updateErr) => {
                            if (updateErr) {
                                return res.status(500).json({ message: 'Failed to update last login', error: updateErr.message, success: false });
                            }

                            console.log("Device last login updated");
                            return res.status(200).json({ message: 'Device last login updated', success: true });
                        }
                    );
                } else {
                    ...
                    db.query(
                        ...
                        (insertErr, result) => {
                            if (insertErr) {
                                return res.status(500).json({ message: 'Database query failed', error: insertErr.message, success: false });
                            }
